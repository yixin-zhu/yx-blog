...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public ResponseResult userInfo() {
        ...
        //获取当前用户的用户id。SecurityUtils是我们在yx-framework工程写的类
        Long userId = SecurityUtils.getUserId();

        //根据用户id查询用户信息
        User user = getById(userId);

        //封装成UserInfoVo(我们在yx-framework工程写的类)，然后返回
    ...
    @Autowired
    //解密和加密用的这套算法。我们在yx-blog工程的securityConfig类里面覆盖过PasswordEncoder这个bean
    ...
    public ResponseResult register(User user) {
        ...
        //在yx-blog工程的securityConfig类里面有解密算法，当时我们写了一个passwordEncoder方法，并且注入到了spring容器
